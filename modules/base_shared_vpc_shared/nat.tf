...
resource "google_compute_router" "nat_router_region1" {
  ...
  name    = "cr-${local.network_name}-${var.default_region1}-nat-router"
...
resource "google_compute_address" "nat_external_addresses_region1" {
  ...
  project = var.project_id
  name    = "ca-${local.network_name}-${var.default_region1}-${count.index}"
  region  = var.default_region1
}

resource "google_compute_router_nat" "egress_nat_region1" {
  count                              = var.nat_enabled ? 1 : 0
  name                               = "rn-${local.network_name}-${var.default_region1}-egress"
  project                            = var.project_id
...
#   count   = var.nat_enabled ? 1 : 0
#   name    = "cr-${local.network_name}-${var.default_region2}-nat-router"
#   project = var.project_id
...
#   project = var.project_id
#   name    = "ca-${local.network_name}-${var.default_region2}-${count.index}"
#   region  = var.default_region2
# }

# resource "google_compute_router_nat" "egress_nat_region2" {
#   count                              = var.nat_enabled ? 1 : 0
#   name                               = "rn-${local.network_name}-${var.default_region2}-egress"
#   project                            = var.project_id
