...
#   secondary_ranges = var.secondary_ranges
  ...
  routes = concat(
    [{
      name              = "rt-${local.network_name}-1000-all-default-private-api"
...
#   name          = "ga-${local.network_name}-vpc-peering-internal"
#   project       = var.project_id
#   purpose       = "VPC_PEERING"
...
module "region1_router1" {
  ...
  version = "~> 0.4.0"
  count   = var.mode != "spoke" ? 1 : 0
  name    = "cr-${local.network_name}-${var.default_region1}-cr1"
  project = var.project_id
...
module "region1_router2" {
  ...
  version = "~> 0.4.0"
  count   = var.mode != "spoke" ? 1 : 0
  name    = "cr-${local.network_name}-${var.default_region1}-cr2"
  project = var.project_id
  network = module.main.network_name
  region  = var.default_region1
  bgp = {
...
#   version = "~> 0.4.0"
#   count   = var.mode != "spoke" ? 1 : 0
#   name    = "cr-${local.network_name}-${var.default_region2}-cr3"
#   project = var.project_id
#   network = module.main.network_name
...
#   version = "~> 0.4.0"
#   count   = var.mode != "spoke" ? 1 : 0
#   name    = "cr-${local.network_name}-${var.default_region2}-cr4"
#   project = var.project_id
#   network = module.main.network_name
#   region  = var.default_region2
#   bgp = {
...
# module "serverless-connector1" {
#   source     = "terraform-google-modules/network/google//modules/vpc-serverless-connector-beta"
#   project_id = var.project_id
#   vpc_connectors = [
 
#     {
#         name          = "sec-core-${var.environment_code}-${var.default_region1}-ac"
#         region        = "${var.default_region1}"
#         network       = module.main.network_name
...
# module "serverless-connector2" {
#   source     = "terraform-google-modules/network/google//modules/vpc-serverless-connector-beta"
#   project_id = var.project_id
#   vpc_connectors = [
 
#     {
#         name          = "sec-core-${var.environment_code}-${var.default_region2}-ac"
#         region        = "${var.default_region2}"
#         network       = module.main.network_name
#         ip_cidr_range = "10.100.2.0/28"
#         subnet_name   = null
#         machine_type  = "e2-micro"
#         min_instances = 2
#         max_instances = 3
